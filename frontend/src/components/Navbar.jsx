...
const Navbar = () => {
  ...
  React.useEffect(()=>{
    console.log("Modal status from Navbar component :", open)
  }, [open])
  return (
    <ModalProvider>
    <nav className="flex flex-row fixed top-3 left-3 right-3 px-[30px] bg-none backdrop-blur-xs max-w-screen text-white justify-between items-center h-[40px] shadow-lg z-50 rounded-lg" id="navbar">
        ...
        <div className="flex flex-row gap-2 pl-[60px]">
            <button className="cursor-pointer hover:text-neutral-300 max-sm:hidden">CV</button>
        </div>
        ...
        <ModalTrigger className="min-sm:hidden">
        Menu
        </ModalTrigger>
    </nav>
    {open && (
      <ModalBody>
        <ModalContent>
            <div className="flex flex-row bg-[#f5f5f5] justify-center items-center rounded-lg min-w-[80%] min-h-[400px] p-[5%]" id="mobile_nav_container">
